		...
		foreach($latest as $post) {
				...
				if ($thumb == 'true') {
					if ( has_post_thumbnail($post->ID) ){
							$output .= '<div class="featured-thumbnail"><a href="'.get_permalink($post->ID).'" title="'.get_the_title($post->ID).'"><span class="img-wrap"></span>';
...
function shortcode_audio($atts, $content = null) {
    ...
    if($mp3 !== false){
        ...
        _e("The URL you entered is not a .mp3 file.", "theme1385");
...
function shortcode_video($atts, $content = null) {
    ...
    if($vimeo !== false){
        ...
        _e("You entered a video URL that isn't compatible with this shortcode.", "theme1385");
