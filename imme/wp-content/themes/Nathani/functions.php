...
$regSettings = array(
	...
	'copyrighttext' => 'Copyright 2012 Nathani', 
...
function redirectWithEscapeFragment(){
	...
	if(isset($_SERVER['REDIRECT_URL'])){
		$script_url = $preHTTP.$_SERVER["SERVER_NAME"].$_SERVER['REDIRECT_URL'].((!empty($_SERVER["QUERY_STRING"]))?addionalCharacter($script_url):'').$_SERVER["QUERY_STRING"];
	}else{
		$script_url = $preHTTP.$_SERVER["SERVER_NAME"].$_SERVER["REQUEST_URI"];
	}
	$script_url = str_replace(site_url().'/','', $script_url);
	header('location:'.site_url().'/?_escaped_fragment_='.$script_url);
	exit;
}
...
if(!is_admin())
{
	...
	wp_enqueue_script("bjqs", $tmpurl."/js/bjqs-1.3.js", false, null); 
        wp_enqueue_script("lbox", $tmpurl."/js/jquery.lightbox-0.5.min.js", false, null);
	if(!$demo)
        ...
        wp_enqueue_style('nivoLightStyle', $tmpurl."/css/nivo-light.css", false, null, 'all');
        wp_enqueue_style('nivoSliderStyle', $tmpurl."/css/nivo-light.css", false, null, 'all');
...
class My_Walker extends Walker_Nav_Menu
{
	function start_el(&$output, $item, $depth, $args) 
	{
		...
		$itemURL = str_replace(site_url().'/','', $itemURL);
